...
const FIRE_PULSE_ON_MS = Number(process.env.ARRAS_FIRE_PULSE_ON_MS || 75);
const FIRE_PULSE_OFF_MS = Number(process.env.ARRAS_FIRE_PULSE_OFF_MS || 325);
...
const MANUAL_CONTROL_INTERVAL_MS = Number(process.env.ARRAS_MANUAL_CONTROL_INTERVAL_MS || 45);
...
class ProtocolOnlyRandomClient {
  constructor(socketUrl, options = {}) {
    ...
    this.lastSentAimX = null;
    this.lastSentAimY = null;
    this.lastControlUpdateAt = 0;
  ...
  sendMovementInput() {
    ...
    // Mouse aim is gameplay input even when neither mouse button nor autofire
    // is active. The old gate dropped these packets, so "follow mouse" bots
    // appeared to ignore the cursor unless firing/spinning.
    const aimX = Number.isFinite(this.controlState.mouseX) ? this.controlState.mouseX : 0;
    const aimY = Number.isFinite(this.controlState.mouseY) ? this.controlState.mouseY : 0;
    const aimChanged =
      this.lastSentAimX === null ||
      this.lastSentAimY === null ||
      Math.abs(aimX - this.lastSentAimX) > 0.5 ||
      Math.abs(aimY - this.lastSentAimY) > 0.5;

    const sendReleaseInput = this.lastInputHadActiveControl && !hasActiveControl;
    const sendAimInput = aimChanged && (now - this.lastControlUpdateAt <= 1000);
    if (!hasActiveControl && !sendReleaseInput && !sendAimInput) {
      return;
    }
    if (!sendReleaseInput && !hasPersistentManualTarget && !sendAimInput && now - this.controlState.updatedAt > 1000) {
      return;
    }
    ...
    this.lastSentAimX = aimX;
    this.lastSentAimY = aimY;
    this.lastControlUpdateAt = now;
